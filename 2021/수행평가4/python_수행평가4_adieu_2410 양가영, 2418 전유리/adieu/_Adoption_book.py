...
from _json_use import UseJSON as json, UseJSON
...
class Adoption_book:
    ...
    def __init__(self):
        # global json = UseJSON()
        self.animals = self.JSON.get_animals_json()    # 등록된 동물들
        self.users = self.JSON.get_user_json()     # 유저 데이터
        #현재 유저
        # 처음 객체 생성 시 무조건 로그인
        # self.test()     # 테스트 코드
    ...
    def sign_up(self, user_input_list, gender): # inputList : Gui 입력 엔트리들 담겨있음
        ...
        self.JSON.set_user_json(self.users)
        print('가입성공')
    ...
    def check_pick_isAccept(self, user):  # 로그인 했을때 신청한 동물들 중에 수락/거절있는지 확인하기
        for index, check in enumerate(self.users[user]['pick_check']):  # 픽체크 폴문 돌리기
            animal = str(self.users[user]['pick_list'][index])     # 분양할 동물 가져오기
            animal = animal.replace("[", "").replace("'","").replace("]","")    # 불필요한 문자 제거

            if check == 1:  # 수락일경우
                writer = self.animals[animal]['user']   # 작성자 가져오기
                self.users[writer]['up_list'].pop(self.users[writer]['up_list'].index(animal))  # 작성자 up_list 삭제
                ...
                self.users[user]['pick_list'].pop(index)  # 신청한 사용자의 신청리스트에서 삭제
                self.users[user]['pick_check'].pop(index)  # nowuser pick_check 삭제
    ...
    def json_update(self):
        self.JSON.set_animals_json(self.animals)
        self.JSON.set_user_json(self.users)
