...
class Jacket_donate:
    def __init__(self, jacket_donate):
        ...
        # jacketnEx 이미지
        self.jacketnEx = tkinter.Button(self.jacket_donate, width=210, height=205, borderwidth=0)
        self.jacketnEx.place(x=150, y=200)
        self.jacketnExImg = tkinter.PhotoImage(file="img/jacket.png")
        self.jacketnEx.config(image=self.jacketnExImg)

        #레이블 박스
        school = ["미림마이스터고", "선린인터넷고", "디지털미디어고"]  # 콤보 박스에 나타낼 항목 리스트
        combobox = ttk.Combobox(jacket_donate)  # root라는 창에 콤보박스 생성
        ...
        #레이블 선택되었음 대기
        school_lb = Label(jacket_donate)  # root라는 창에 레이블 생성
        school_lb.place(x=670, y=148) # 레이블 배치

        #size 라디오
        size_lang_var = StringVar()  # str 형으로 변수 저장
        btn_size1 = Radiobutton(jacket_donate, text="S ", value="S", variable=size_lang_var)
        ...
        btn_gender1 = Radiobutton(jacket_donate, text="남", value="남", variable=gender_lang_var)
        ...
        # back 버튼
        self.backButton = tkinter.Button(self.jacket_donate, width=130, height=75, borderwidth=0, command=self.BackButton)
