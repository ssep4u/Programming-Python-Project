...
class Menu1_3:
    def __init__(self, menu1_3):
        self.menu1_3 = menu1_3
        ...
        self.title = tkinter.Label(self.menu1_3, text=" ♡ 오므라이스 ♡ ", foreground="#9b95b7", font=self.titleFont)
        self.title.place(x=280, y=40)

        # 데이터 정보 불러오기
        self.info = open("menu1_3.txt", "r", encoding="utf-8")
        addY = 150
        while True:
            ...
            for self.line in self.infoData:
                self.title = tkinter.Label(self.menu1_3, text=self.line, foreground="#9b95b7")
        ...
        self.homeButton = tkinter.Button(self.menu1_3, text="home", image=self.home, command=self.moveHomeButton,
                                         repeatdelay=20)
        self.homeButton.place(x=50, y=400)

        # back 버튼
        self.back = tkinter.PhotoImage(file="image/back.gif")
        self.backButton = tkinter.Button(self.menu1_3, text="home", image=self.back, command=self.moveBackButton,
                                         repeatdelay=20)
        self.backButton.place(x=710, y=400)

    # main=home 페이지로 가기
    def moveHomeButton(self):
        startMove = start.Start(self.menu1_3)

    # back 페이지로 뒤로가기
    def moveBackButton(self):
        backMove = kang1.Kang1(self.menu1_3)