...
class Menu2_2:
    def __init__(self, menu2_2):
        self.menu2_2 = menu2_2
        ...
        self.title = tkinter.Label(self.menu2_2, text=" ♡ 웃기는 짜장 떡볶이 ♡ ", foreground="#9b95b7", font=self.titleFont)
        self.title.place(x=220, y=40)

        # 데이터 정보 불러오기
        self.info = open("menu2_2.txt", "r", encoding="utf-8")
        addY = 150
        while True:
            ...
            for self.line in self.infoData:
                self.title = tkinter.Label(self.menu2_2, text=self.line, foreground="#9b95b7")
        ...
        self.homeButton = tkinter.Button(self.menu2_2, text="home", image=self.home, command=self.moveHomeButton,
                                         repeatdelay=20)
        self.homeButton.place(x=50, y=400)

        # back 버튼
        self.back = tkinter.PhotoImage(file="image/back.gif")
        self.backButton = tkinter.Button(self.menu2_2, text="home", image=self.back, command=self.moveBackButton,
                                         repeatdelay=20)
        self.backButton.place(x=710, y=400)

    # main=home 페이지로 가기
    def moveHomeButton(self):
        startMove = start.Start(self.menu2_2)

    # back 페이지로 뒤로가기
    def moveBackButton(self):
        backMove = kang2.Kang2(self.menu2_2)